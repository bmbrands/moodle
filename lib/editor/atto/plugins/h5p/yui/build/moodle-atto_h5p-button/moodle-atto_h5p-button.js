    ...
    TEMPLATE = '' +
            '<form class="atto_form">' +
                ...
                '<ul class="nav nav-tabs mb-1" role="tablist">' +
                    '<li class="nav-item">' +
                        '<a class="nav-link {{embedActive}}" href="#{{elementid}}_{{CSS.INPUTH5PURL}}" ' +
                            'role="tab" data-toggle="tab">' +
                            '{{get_string "h5purl" component}}' +
                        '</a>' +
                    '</li>' +
                    '<li class="nav-item">' +
                        '<a class="nav-link {{fileActive}}" href="#{{elementid}}_{{CSS.H5PBROWSER}}" ' +
                            'role="tab" data-toggle="tab">' +
                ...
                '<div class="tab-content">' +
                    '<div class="tab-pane {{embedActive}}" id="{{elementid}}_{{CSS.INPUTH5PURL}}">' +
                '{{/if}}' +
                '{{#if canEmbed}}' +
                '<div class="mb-4">' +
                    '<label for="{{elementid}}_{{CSS.INPUTH5PURL}}">{{get_string "enterurl" component}}</label>' +
                    '<input class="form-control fullwidth {{CSS.INPUTH5PURL}}" type="url" value="{{embedURL}}" ' +
                    'id="{{elementid}}_{{CSS.INPUTH5PURL}}" size="32"/>' +
                '</div>' +
                '{{/if}}' +
                '{{#if canUploadAndEmbed}}' +
                    '</div>' +
                    '<div class="tab-pane {{fileActive}}" id="{{elementid}}_{{CSS.H5PBROWSER}}">' +
                '{{/if}}' +
                '{{#if canUpload}}' +
                '<div class="mb-4">' +
                    ...
                    '<div class="form-check">' +
                        '<input type="checkbox" {{optionDisplayButtons}} ' +
                    ...
                    '</div>' +
                    '<div class="form-check">' +
                        '<input type="checkbox" {{optionAllowDownload}} ' +
        ...
        H5PEXTERNAL = '' +
            '<div class="position-relative h5p-embed-placeholder">' +
                ...
                '<iframe id="h5pcontent" class="h5pcontent" src="{{url}}/embed" ' +
                    'data-type="embed" width="100%" height="637" frameborder="0"' +
        ...
        H5PFILE = '' +
            '<div class="position-relative h5p-file-placeholder">' +
                '<div class="attoh5poverlay"></div>' +
                '<iframe id="h5pcontent" class="h5pcontent" ' +
                    'src="' + M.cfg.wwwroot + '/h5p/embed.php?url={{h5pfile}}' +
                    '{{#if optionDisplayButtons}}&frame=1{{/if}}' +
                    '{{#if optionAllowDownload}}&export=1{{/if}}' +
                    ...
                    'data-frame="{{#if optionDisplayButtons}}=1{{/if}}" ' +
                    'data-export="{{#if optionAllowDownload}}=1{{/if}}" ' +
...
Y.namespace('M.atto_h5p').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _getDialogueContent: function() {
        ...
        if (this._placeholderH5P) {
            ...
            if (type === 'file') {
                ...
                optionDisplayButtons = (this._placeholderH5P.getAttribute('data-frame') ? 'checked' : '');
                optionAllowDownload = (this._placeholderH5P.getAttribute('data-export') ? 'checked' : '');
                ...
                embedURL = this._placeholderH5P.getAttribute('src');
            }
        }
    ...
    _setEventListeners: function() {
        ...
        if (permissions.canUpload) {
            ...
            form.one(SELECTORS.OPTION_DISPLAY_BUTTONS).on('change', function() {
                if (!form.one(SELECTORS.OPTION_DISPLAY_BUTTONS).get('checked')) {
                    form.all('.check-depend').set('checked', false);
                    form.all('.check-depend').set('disabled', true);
                } else {
                    form.all('.check-depend').set('disabled', false);
                }
            }, this);
        }
