...
var CSS = {
        ...
        INPUTH5PHASH: 'atto_h5p_hash',
        INPUTH5PSIZE: 'atto_h5p_size',
    ...
    SELECTORS = {
        ...
        INPUTH5PHASH: '.' + CSS.INPUTH5PHASH,
        INPUTH5PSIZE: '.' + CSS.INPUTH5PSIZE,
    ...
    TEMPLATE = '' +
            '<form class="atto_form mform" id="{{elementid}}_atto_h5p_form">' +
                ...
                '{{#if canUpload}}' +
                '<div class="mb-4">' +
                    '<input class="{{CSS.INPUTH5PSIZE}}" name="size" type="hidden" value=""> ' +
                    '<input class="{{CSS.INPUTH5PHASH}}" name="hash" type="hidden" value=""> ' +
...
Y.namespace('M.atto_h5p').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _filepickerCallback: function(params) {
        if (params.url !== '') {
            ...
            var wsUrl = M.cfg.wwwroot + '/lib/editor/atto/plugins/h5p/ajax.php';
            var wsParams = {
    ...
    /**
     * Set the file parameters in the Dialogue form.
     *
     * @param {String} id
     * @param {EventFacade} params
     * @method _setFileParams
     * @private
     */
    _setFileParams: function(id, params) {
        var response = JSON.parse(params.responseText);
        if (response.contenthash) {
            this._form.one(SELECTORS.INPUTH5PHASH).set('value', response.contenthash);
        }
        if (response.size) {
            this._form.one(SELECTORS.INPUTH5PSIZE).set('value', response.size);
        }
    ...
    _setH5P: function(e) {
        ...
        if (url !== '') {
            ...
            options.filehash = form.one(SELECTORS.INPUTH5PHASH).get('value');
            options.filesize = form.one(SELECTORS.INPUTH5PSIZE).get('value');
    ...
    ATTRS: {
        ...
        /**
         * The contextid to use when requesting webservice info.
         *
         * @attribute contextid
         * @type Int
         */
